...
def historialporpaciente():
    def buscar():
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT hc.fecha, hc.hora, a.descripcion, m.nombre, hc.detalles_sintomas
            FROM historia_clinica hc
            JOIN paciente p ON p.id = hc.paciente_id
            JOIN medico m ON m.id = hc.medico_id
            JOIN area a ON a.id = hc.area_id
            WHERE p.dni = ?
        ...
        if resultados:
            ...
            messagebox.showinfo("Sin resultados", "No se encontró historial para ese DNI.")
    ...
    columnas = ("Fecha", "Hora", "Área", "Médico", "Síntomas")
